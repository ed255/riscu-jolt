...
#[derive(Clone, Debug)]
pub struct MulTable<F>(PhantomData<F>);
...
/// `MUL`'s evaluation result is from its own calculation on finite field
/// this table does range check
/// T[z] = T_1[z_1] + T_2[z_2] * 2^8 + ... + T_8[z_8] * 2^56
impl<F: PrimeField> DecomposableTable<F> for MulTable<F> {
